...
export default function Home() {
  ...
  const fetchResults = useCallback(async (query: string, mode: 'drug' | 'ingredient', page: number) => {
    ...
    try {
      ...
      setError(isNetworkError ? '네트워크 연결을 확인해주세요.' : '검색 중 오류가 발생했습니다. 다시 시도해주세요.');
      setResults(null);
    } finally {
  ...
  useEffect(() => {
    ...
    if (currentQuery) {
      ...
      setMeta(
        `${currentQuery} - Pill Trace`,
        `${currentQuery} ${currentMode === 'ingredient' ? '성분이 포함된 의약품' : '의약품'} 검색 결과 - 성분, 효능, 주의사항 확인`,
      );
    } else {
      setHasSearched(false);
      setResults(null);
      setError('');
      setMeta('Pill Trace - 의약품 성분 검색', '약 이름이나 성분으로 의약품을 검색하고, 동일/유사 성분의 약을 찾아보세요. 공공데이터포털 의약품 허가정보 기반.');
    }
    return () => {
      abortControllerRef.current?.abort();
      if (slowTimerRef.current) clearTimeout(slowTimerRef.current);
    ...
    <div>
      ...
      {!showHero && (
        <div className="max-w-6xl mx-auto px-4 py-8" role="region" aria-label="검색 결과" aria-busy={isLoading}>
          ...
          {error && !isLoading && (
            <div className="mt-4 text-center py-8 bg-red-50 rounded-2xl border border-red-100" role="alert">
              <svg className="w-10 h-10 text-red-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {results && !isLoading && (
            <div>
              ...
              {results.items.length === 0 ? (
                <div className="text-center py-16 bg-gray-50 rounded-2xl border border-gray-100" role="status">
